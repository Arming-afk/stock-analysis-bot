...
### What gets published — this repository is public

The full report contains the entire financial position: share counts, cost
basis, unrealized P/L, portfolio value, cash, and an LLM rationale written with
all of that in context. **None of it is committed.**

The scheduled run writes the full report to `data/reports/` (gitignored, dies
with the runner). [`tools/publish.py`](tools/publish.py) then rebuilds a public
copy from an explicit **whitelist** — a blacklist would silently leak whatever
field is added next, a whitelist fails closed — and only that copy reaches
`docs/`.

| Published | Withheld |
|---|---|
| signal, rule, price, fair value, valuation gap | share counts, cost basis, unrealized P/L |
| full DCF internals and gate verdict | portfolio value, cash |
| news sentiment, sources, headlines | ticker and sector weights |
| confidence score, whether a position is held | the original LLM rationale |

The published rationale is regenerated by deterministic code from the redacted
fields, rather than trying to scrub money out of free text. The workflow also
greps the actual bytes before committing and fails the run if a position key
reappears, and [`tests/test_publish.py`](tests/test_publish.py) walks the
payload recursively to catch a regression in `publish.py` itself.

If you want the full report on the dashboard, the repository has to be private
— host it on Cloudflare Pages and pass `--no-redact` to `publish.py`.
